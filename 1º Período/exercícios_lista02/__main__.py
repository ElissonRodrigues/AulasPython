...
files = sorted(glob("exercícios_lista02/LISTA02/*.py"))
...
for name in files:
	with open(name) as a:
		...
		exec (f"from exercícios_lista02.LISTA02.{plugin_name} import {plugin_name}")
...
def executar_tudo():
	...
	print ("\nRetornando ao menu principal..."); sleep(2)
...
def main():
	...
	try:		
		if type(execution_value) is int and len(str(execution_value)) <= 2 and execution_value > 0 and not execution_value > len(files) or type(execution_value) is str:
			...
			os.system('cls' if os.name == 'nt' else 'clear')
			print ("\n\nVerifique o exercício digitando e tente novamente.")
			sleep(2);return
	except:
		print (format_exc())
